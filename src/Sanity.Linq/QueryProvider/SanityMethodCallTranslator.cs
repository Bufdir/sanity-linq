    ...
    bool isTopLevel = false)
{
    ...
    private string AddInclude(string fieldPath, Type propertyType, string? sourceName, Type? originalType = null)
    {
        ...
        // If this is a filtered included (contains [filter]), ensure the base field is also included
        ...
        if (openBracketIndex <= 0) return fieldPath;
    ...
    private string HandleAny(MethodCallExpression e)
    {
        ...
        if (SanityMethodCallTranslatorHelper.TryGetLambda(e, 1, out var lambda))
        {
            ...
            if (collectionExpr.Type != typeof(string) && typeof(IEnumerable).IsAssignableFrom(collectionExpr.Type))
            {
                ...
                return SanityMethodCallTranslatorHelper.WrapWithCountGreaterThanZero(filter);
            }
        }

        var op = e.Arguments.Count > 0 ? transformOperand(e.Arguments[0]) : SanityConstants.AT;
        return SanityMethodCallTranslatorHelper.WrapWithCountGreaterThanZero(op);
    }
    ...
    private string HandleOrdering(MethodCallExpression e, bool descending)
    {
        ...
        if (!SanityMethodCallTranslatorHelper.TryGetLambda(e, 1, out var lambda) || !isTopLevel || queryBuilder.IsSilent)
            return string.Empty;
    ...
    private string? HandlePropertyContainsProperty(Expression collectionExpr, Expression valueExpr)
    {
        ...
        if (collUnwrapped is not MemberExpression || valUnwrapped is not MemberExpression) return null;
    ...
    private string HandleSkip(MethodCallExpression e)
    {
        ...
        if (e.Arguments.Count <= 1 || e.Arguments[1] is not ConstantExpression c) return string.Empty;
    ...
    private bool TryHandleAnyConstantCollection(MethodCallExpression e, LambdaExpression lambda, [NotNullWhen(true)] out string? result)
    {
        ...
        if (collectionExpr is not ConstantExpression expression || lambda.Body is not BinaryExpression { NodeType: ExpressionType.Equal } be) return false;
        ...
        if (paramSide == null || target == null) return false;
        if (expression.Value is not IEnumerable collection || collection is IQueryable) return false;

        // Extract values from a collection based on paramSide
