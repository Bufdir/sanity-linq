...
public class SanityHtmlSerializers
{
    ...
    public Task<string> SerializeDefaultBlockAsync(JToken input, SanityOptions sanity, object? context = null)
    {
        ...
        if (input["listItem"]?.ToString() == "bullet")
        {
            ...
            var firstItem = (bool?)input["firstItem"];
            if (firstItem == true)
            {
                listStart.Append("<ul>");
            }
            var lastItem = (bool?)input["lastItem"];
            if (lastItem == true)
            {
                listEnd.Append("</ul>");
            }
        ...
        if (input["children"] != null)
        {
            foreach (var child in input["children"]!)
            {
                ...
                if (childMarks is { HasValues: true })
                {
                    foreach (var mark in childMarks!)
                    {
                        ...
                        if (markDef != null)
                        {
                            ...
                            {
                                start.Append($"<a target=\"_blank\" href=\"{markDef["href"]}\">");
                                end.Append("</a>");
                            }
                            else if (markDef["_type"]?.ToString() == "internalLink")
                            {
                                start.Append($"<a href=\"{markDef["href"]}\">");
                                end.Append("</a>");
                            }
                            // Mark not supported....
                        }
                ...
                text.Append(start.ToString() + child["text"] + end);
            }
        }
