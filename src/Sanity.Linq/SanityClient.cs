...
public class SanityClient
{
    ...
    public void Initialize()
    {
        ...
        if (Options.UseCdn)
        {
            _httpQueryClient.BaseAddress = new Uri($"https://{WebUtility.UrlEncode(Options.ProjectId)}.apicdn.sanity.io/{Options.ApiVersion}/");
        }
        else
        {
            _httpQueryClient.BaseAddress = new Uri($"https://{WebUtility.UrlEncode(Options.ProjectId)}.api.sanity.io/{Options.ApiVersion}/");
        }
    ...
    protected virtual async Task<TResult> CommitMutationsInternalAsync<TResult>(object mutations, bool returnIds = false, bool returnDocuments = false, SanityMutationVisibility visibility = SanityMutationVisibility.Sync, CancellationToken cancellationToken = default)
    {
        ...
        var json = mutations is string s ? s : 
            mutations is SanityMutationBuilder builder ? builder.Build(SerializerSettings) : 
    ...
    protected virtual async Task<TResponse> HandleHttpResponseAsync<TResponse>(HttpResponseMessage response)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            try
            {
                ...
                if (obj == null)
                {
                    throw new Exception("Failed to deserialize Sanity response: deserialized to null");
                }
                return obj;
            }
            catch (Exception ex)
